...
```mermaid
flowchart LR
    A["Self Green Campaign"] --> B["Activity Reporting via app"]
    B --> C{"Verification by Admin"}
    
    C -->|if post follow guideline| D["Add Green Coin to user"]
    C -->|if post complete| E["Add SAT Points"]
    C -->|if post reject| F["Activity Rejection"]
    
    D --> G["Auto ranked on leaderboard"]
    G --> H["Top Ranked yearly nominated for BEKEN Award"]
    
    E --> I["Auto ranked on semester Activity Ranking"]
    
    F --> J["Send notification to user"]
```
