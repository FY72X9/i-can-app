...
```mermaid
flowchart TD
    SUB["Laporan Masuk (Foto, GPS, Link Medsos)"] --> AI["Gemini AI Visual Pre-Scan"]
    AI --> DEC{"Evaluasi Verifikator SSO/TFI"}
    
    DEC -->|"Aksi Fisik Lengkap + Sesuai Guideline"| AP_FULL["✅ Approve Full (+SAT Points, +Comserv Hours, +Green Coins)"]
    DEC -->|"Hanya Kampanye Medsos / Aksi Harian"| AP_COIN["🪙 Approve Coins Only (+Green Coins untuk BEKEN Leaderboard)"]
    DEC -->|"Foto Buram / Tidak Ada Almamater / Hoaks"| REJ["❌ Reject with Reason (Catatan Formatif untuk Pengajuan Ulang)"]
```
