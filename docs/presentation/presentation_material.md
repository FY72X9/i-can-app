...
```mermaid
flowchart LR
    A["Mahasiswa Melakukan Aksi<br/>(Pohon, Biopori, Edukasi)"] -->|Upload Manual / Formulir| B["Penumpukan Antrean Verifikasi<br/>(Verifikator SSO & TFI Overload)"]
    B -->|Waktu Review 2-4 Minggu| C["Mahasiswa Kehilangan Minat &<br/>Potensi Laporan Palsu"]
    C -->|Data Tercecer| D["Capaian SDG & Karbon Kampus<br/>Tidak Terekam Optimal"]
```
...
```mermaid
graph TD
    subgraph SDT["1. Self-Determination Theory (Deci & Ryan)"]
        SDT1["Motivasi Intrinsik (Peduli Lingkungan)"] --> DUAL["Dual-Track Reward Architecture"]
        SDT2["Motivasi Ekstrinsik (Kredit Akademik SAT)"] --> DUAL
    end

    subgraph TTF["2. Task-Technology Fit (Goodhue & Thompson)"]
        TTF1["Pelaporan Lapangan Cepat"] --> PWA["PWA + Kompresi Canvas + AI Vision"]
        TTF2["Validasi Otomatis Foto & Metadata"] --> PWA
    end

    subgraph ISS["3. D&M IS Success Model (DeLone & McLean)"]
        ISS1["Kualitas Informasi (Hitungan Karbon IPCC)"] --> TRUST["Kepercayaan & Akuntabilitas Kampus"]
        ISS2["Kualitas Sistem (Kecepatan <1s & Keamanan RLS)"] --> TRUST
    end
```
...
```mermaid
flowchart TD
    subgraph UserSide["📱 1. Mahasiswa (PWA Mobile)"]
        U1["Registrasi Email BINUS & Login Cepat"]
        U2["Dynamic Report Form & Drag-Drop Foto"]
        U3["Portofolio Transkrip SAT & Saldo Green Coins"]
        U4["Feed Komunitas & Storytelling Publik"]
    end

    subgraph AIService["🤖 2. Intelligence Core (AI Vision)"]
        AI1["Gemini 1.5 Flash Vision Multimodal"]
        AI2["Deteksi Jas Almamater & Objek Fisik"]
        AI3["Pengecekan Tagar Resmi & Rekomendasi Skor"]
    end

    subgraph AdminSide["🏛️ 3. Pengelola Kampus (SSO/TFI & Super Admin)"]
        A1["Portal Triage Verifikator (Review Aksi Cepat)"]
        A2["Super Admin Panel (AdminLTE Enterprise View)"]
        A3["Manajemen Notifikasi & Export Transkrip Kegiatan"]
    end

    UserSide --> AIService --> AdminSide
```
...
```mermaid
flowchart TD
    subgraph Input["Laporan Aksi Mahasiswa"]
        A["Unggah Laporan Aksi / Kampanye<br/>(Foto Lapangan, GPS, Link Medsos/Video, Tagar Resmi)"]
    end

    subgraph Verification["Mesin Verifikasi Dual-Engine"]
        A --> B["Gemini 1.5 Flash AI Pre-Scan"]
        B --> C{"Keputusan Verifikator SSO / TFI"}
    end

    subgraph TrackA["Track 1: Gamifikasi & Reputasi (Sosial)"]
        C -->|"Kesesuaian Guideline Kampanye"| D["+Green Coins (GC)"]
        D --> E["Peringkat di Green Leaderboard"]
        E --> F["🏆 Nominasi Tahunan BEKEN Award<br/>(BINUS Eco-Ksatria Environmental Network)"]
    end

    subgraph TrackB["Track 2: Rekognisi Akademik Resmi (Regulasi)"]
        C -->|"Aksi Fisik Nyata Terverifikasi Lengkap"| G["+Poin SAT & Jam Community Service Riil"]
        G --> H["📄 Transkrip Portofolio myBINUS & TFI Apps"]
    end

    subgraph TrackC["Track 3: Feedback Perbaikan"]
        C -->|"Belum Memenuhi Syarat"| I["Penolakan dengan Catatan Evaluasi Konstruktif"]
    end

    style TrackA fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    style TrackB fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
    style TrackC fill:#ffebee,stroke:#c62828,stroke-width:2px
```
...
flowchart TB
    subgraph Client["📱 Frontend Client (React 18 + Vite + Tailwind CSS)"]
        UI["Antarmuka 8 Layar PWA"]
        Canvas["Kompresor Canvas HTML5 (<200KB WebP)"]
        Auth["Web Crypto SHA-256 Auth / Logto OIDC"]
        Store["State Management (Zustand)"]
    end

    subgraph EdgeAI["⚡ Edge AI & Service Worker"]
        Gemini["Google Gemini 1.5 Flash Multimodal Vision"]
        Cache["Local Offline Storage & Cache"]
    end

    subgraph Cloud["🗄️ Backend Cloud (Supabase Managed Engine)"]
        Postgres[(PostgreSQL 15 + Row Level Security)]
        StorageBucket["Storage: action-photos"]
        RealtimeSub["Supabase Realtime Engine"]
    end

    subgraph Portal["🏛️ Portal Administrasi"]
        AdminView["Portal Verifikator TFI/SSO & AdminLTE"]
        ExportEngine["Mesin Sinkronisasi myBINUS / TFI Apps"]
    end

    UI --> Canvas --> StorageBucket
    UI --> Store --> Postgres
    UI --> Auth
    Canvas --> Gemini
    Gemini --> AdminView
    AdminView --> Postgres
    Postgres --> RealtimeSub --> UI
    Postgres --> ExportEngine
```
...
sequenceDiagram
    ...
    participant App as 📱 I-CAN PWA
    participant AI as 🤖 Gemini AI
    participant Admin as 🛡️ Verifikator SSO/TFI
    participant Feed as 🌐 Feed Komunitas & Medsos

    M->>App: Pindai QR Banner di Kampus / Login Email BINUS
    M->>App: Pilih Jenis Kegiatan (Formulir Otomatis Menyesuaikan)
    M->>App: Unggah Foto (Auto-kompres <200KB) + Cantumkan Link Medsos
    App->>AI: Pengecekan AI Otomatis (Objek, Almamater, Tagar Resmi)
    AI-->>App: Skor Rekomendasi Kesesuaian (Akurasi: 94%)
    App->>Admin: Laporan Masuk Antrean Verifikasi TFI/SSO
    Admin->>App: 1-Klik Setujui (Approve Full SAT / Approve Coins)
    App->>M: Notifikasi Masuk: Poin SAT & Saldo Green Coins Bertambah
    App->>Feed: Publikasikan Kartu Aksi ke Galeri Feed Kampus
```
...
```mermaid
flowchart TD
    subgraph R1["1. Risiko Manipulasi & Plagiarisme"]
        F1["Penggunaan Foto Berulang / Unduhan Google"] --> M1["Deteksi Anomali Gemini AI + Image Perceptual Hash"]
        F2["Pelaporan Lokasi Fiktif"] --> M2["Pencatatan Geolokasi Browser & Verifikasi Metadata"]
    end

    subgraph R2["2. Risiko Beban Verifikator (Burnout)"]
        F3["Lonjakan Ribuan Laporan di Akhir Semester"] --> M3["AI Pre-Scoring & Prioritas Triage 1-Klik Otomatis"]
    end

    subgraph R3["3. Risiko Inflasi Tokenomics"]
        F4["Kecurangan Akumulasi Koin"] --> M4["Isolasi Dual-Track: Green Coins Tidak Bisa Ditukar ke Nilai Akademik"]
    end

    subgraph R4["4. Risiko Ketergantungan Layanan Cloud"]
        F5["Kenaikan Kuota Penggunaan Layanan"] --> M5["Arsitektur Bebas Keterikatan (Bisa Dipindah ke Server Lokal BINUS)"]
    end
```
...
```mermaid
gantt
    title Roadmap Eksekusi I-CAN (2026 - 2027)
    dateFormat  YYYY-MM-DD
    section Fase 1: Peluncuran MVP (2026)
    Penyempurnaan Arsitektur & Dual-Track :done, 2026-08-01, 2026-09-30
    Uji Coba Terbatas (Alam Sutera & Kemanggisan) :active, 2026-10-01, 2026-11-15
    Rilis Output MVP (Target Q4 2026) :crit, 2026-11-15, 2026-12-31
    section Fase 2: Integrasi & Multi-Kampus (2027 H1)
    Integrasi API myBINUS & TFI Apps :2027-01-01, 2027-03-31
    Ekspansi Kampus (Bandung, Malang, Semarang) :2027-04-01, 2027-06-30
    Penganugerahan Perdana BEKEN Award 2027 :2027-06-01, 2027-06-30
    section Fase 3: ESG Enterprise & Replikasi (2027 H2)
    Dashboard Analitik ESG Dewan Pimpinan :2027-07-01, 2027-09-30
    Paket Adopsi Jaringan Kampus Nasional :2027-10-01, 2027-12-31
```
